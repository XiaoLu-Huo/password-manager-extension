...
- **快速搜索** — 通过 Popup 弹窗搜索密码库中的凭证（开发中）
- **自动填充** — 将匹配的用户名和密码填入登录表单（开发中）
...
3. 开启右上角「开发者模式」
4. 点击「加载已解压的扩展程序」
5. 选择项目根目录（包含 `manifest.json` 的目录），注意需要将 `manifest.json` 复制到 `dist/` 或直接选择项目根目录
...
- **Content Script** — 注入到所有网页，检测登录表单并通知 Service Worker
- **Service Worker** — 后台常驻，作为 API 请求代理，管理 Session Token
- **API Client** — 封装与后端的 HTTP 通信，统一处理响应解析、401 拦截、网络错误
- **Popup UI** — 点击扩展图标弹出的界面（Task 12 开发中）
